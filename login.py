...
USUARIOS = [
    {
        "usuario": "pietro",
        "senha": hashlib.sha256("Pietro28*".encode()).hexdigest(),
        "nivel": "dev",
        "bots": ["pz", "pk", "pg"]
    },
    {
    ...
    {
        "usuario": "usuario1",
        "senha": hashlib.sha256("senha1".encode()).hexdigest(),
        "nivel": "basico",
        "bots": ["pz"]
    }
]
...
def login():
    ...
    user_data = verificar_usuario(usuario, senha)
...
# --- Escolher destino após login ---
def pergunta_area(user_data):
    usuario = user_data["usuario"]
    nivel = user_data["nivel"].strip().lower()
    print(f"Nível do usuário recebido: '{nivel}'")
...
def abrir_edicao_usuarios(usuario_logado, nivel_logado):
    ...
    def adicionar_usuario():
        ...
        if nivel == "dev":
            if any(u['nivel'] == 'dev' for u in USUARIOS):
    ...
    tk.Button(frame, text="Editar", command=editar_usuario).grid(row=0, column=1, padx=5)
